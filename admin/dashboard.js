...
// Submissions are public, unauthenticated user input — never trust it in innerHTML.
...
async function loadAllSubmissions() {
    ...
    loadAndDisplayData('trial', 'trial-requests-body', (item) => `
        <tr class="bg-gray-800 border-b border-gray-700 hover:bg-gray-700/50">
            ...
            <td class="px-6 py-4 font-medium text-white">${escapeHtml(item.name)}</td>
            <td class="px-6 py-4">${escapeHtml(item.email)}</td>
            <td class="px-6 py-4">${escapeHtml(item.device)}</td>
            <td class="px-6 py-4">${escapeHtml(item.country)}</td>
            <td class="px-6 py-4">
    ...
    loadAndDisplayData('contact', 'contact-messages-body', (item) => `
        <tr class="bg-gray-800 border-b border-gray-700 hover:bg-gray-700/50">
            <td class="px-6 py-4">${formatDate(item.timestamp)}</td>
            <td class="px-6 py-4 font-medium text-white">${escapeHtml(item.email)}</td>
            <td class="px-6 py-4 max-w-sm truncate" title="${escapeHtml(item.message)}">${escapeHtml(item.message)}</td>
            <td class="px-6 py-4">
    ...
    loadAndDisplayData('channelList', 'channel-list-body', (item) => `
        <tr class="bg-gray-800 border-b border-gray-700 hover:bg-gray-700/50">
            <td class="px-6 py-4">${formatDate(item.timestamp)}</td>
            <td class="px-6 py-4 font-medium text-white">${escapeHtml(item.email)}</td>
            <td class="px-6 py-4">${escapeHtml(item.preferredRegion) || 'N/A'}</td>
            <td class="px-6 py-4">
